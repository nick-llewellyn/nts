...
| Job | Cost | Purpose |
|-----|------|---------|
| `changes` | ~5 s | Classifies the diff via `dorny/paths-filter`; outputs `rust`, `bindings`, `dart`, `ci`, `docs`, `hooks`, `auggie_hooks`, and `android` flags consumed by the gates below (`docs` gates `doc-snippets`; `hooks` gates the two git-hook jobs; `auggie_hooks` gates the Auggie hook matrix; `android` gates the KGP gate matrix). Always runs. |
| `build` | ~3–5 min × 2 | Dart format / analyze / `flutter test --coverage` on the oldest buildable SDK (3.38.10 — above the declared `>=3.38.0` floor; see the comment on the `build` job) and the latest `stable` channel (matches `.fvmrc`). Also resolves, analyzes, and tests the example app, then smoke-runs the three `example/bin/` CLI scripts under `dart run` with `--mock` — those are the package's only non-Flutter consumers, so running them through `dart` rather than `flutter` is what catches a `dart:ui` import leaking back into the library surface, and the step also pins `nts_cli`'s no-host exit code at 64 so that invocation cannot become a silent success. It does not reach the native-reuse latch path, which needs a real dylib and a failing initialization. Gated on `dart`/`rust`/`bindings`/`ci` (skips on doc-only diffs). Stable-leg uploads `coverage/lcov.info` as a workflow artifact and to Codecov via OIDC. |
...
| `dependency-review` | ~10 s | PR-only supply-chain gate via `actions/dependency-review-action`; fails on `moderate`-or-higher advisories across pubspec + Cargo.toml. Also enforces the NTS-72 SPDX `allow-licenses` list, kept in lockstep with `[licenses].allow` in `rust/deny.toml`. That list is a *distribution* policy — what may be linked into the published package or the `nts_rust` cdylib. Because this action also walks the workflow dependency graph (`pkg:githubactions/...`), which `cargo-deny` never sees, build-time actions are exempted individually via `allow-dependencies-licenses` rather than by widening `allow-licenses`: an action runs on an ephemeral runner and is never conveyed to a user, so its licence imposes no obligation on anything shipped. Currently two such entries: `Swatinem/rust-cache` (LGPL-3.0) and `SonarSource/sonarqube-scan-action` (LGPL-3.0, added for the `github-actions` group bump in Dependabot PR #362). The list carries three non-action entries too — `flutter_rust_bridge`, `code_assets`, and `file_picker` — for a different reason than the action entries: none of the three needs a licence exception, since all are MIT or BSD-3-Clause and already allowed, but pub.dev publishes the field as a lowercase non-SPDX string (`mit`, `bsd-3-clause`), which the action cannot validate and so fails closed on. `file_picker` was added for the 13.1.0 bump in Dependabot PR #361 and hand-verified against the upstream `LICENSE` at its release tag. Note that `allow-dependencies-licenses` is package-scoped, not error-scoped: a listed package is dropped from licence evaluation entirely, so any licence a future release declares would pass unexamined — re-verify by hand on each exact-pin bump. Adding to that carve-out is not a reason to touch `allow-licenses` or `deny.toml`. |
...
| `auggie-hooks` | ~7–8 min × 2 | Runs the three Auggie bead-hook suites under `.augment/hooks/test/` (`classification_test.sh`, `lock_test.sh`, `prime_test.sh`) on both `ubuntu-latest` and `macos-latest`, after an executable-bit check on the two entrypoints `.augment/settings.json` runs directly (`beads-prime.sh`, `beads-sync.sh`), `bash -n` over every hook script and `py_compile` over `lib/command_scan.py`. The suites stub `bd` and run against scratch workspaces under `$TMPDIR`, so no bead store is touched. The macOS leg is what makes this a bash 3.2 compatibility check: the suites are invoked as `/bin/bash …` explicitly, and they propagate that interpreter to the hooks under test. `shfmt` is installed unpinned and via a different installer per platform (apt vs brew) on purpose, so the scanner's parser output is exercised across the versions that differ. Gated on `auggie_hooks`/`ci`/`workflow_dispatch`; `auggie_hooks` is a separate filter key from `hooks` so `tool/hooks/**` and `.augment/**` diffs do not trigger each other's jobs. |
| `auggie-hooks-gate` | ~5 s | Single-name aggregator (`Auggie hook tests gate`) over the `auggie-hooks` matrix, structurally identical to `build-gate`: `needs: [changes, auggie-hooks]` + `if: always()`, passes when `changes` succeeded and the matrix is `success` or `skipped`, fails otherwise. Exists for the same matrix-skip naming reason. Intended required-status-check entry on `main` once registered (see the branch-protection table). |
...
so the eight required checks (`Detect changed paths`, `Dart tests
gate`, `Verify FRB bindings are in sync`, `Rust build + tests +
coverage`, `Hooks shell-syntax check`, `Hooks behaviour check`,
`Android KGP gate matrix`, `Auggie hook tests gate`) resolve green on
doc-only diffs even though `build`, `rust`, `rust-bridge-sync`,
`hooks-syntax`, `hooks-behaviour`, `android-kgp-gate`, and
`auggie-hooks` all skip.
...
(cd example \
  && dart run bin/nts_cli.dart --help > /dev/null \
...
# Same run, no hosts: must exit with the usage code 64, not 0
(cd example && dart run bin/nts_cli.dart --mock > /dev/null 2>&1
 [ $? -eq 64 ])
...
# Auggie bead hooks (any change to .augment/**); needs jq, shfmt,
# python3. /bin/bash, not bash: on macOS that is the 3.2 the hooks
# support. The lock suite takes ~7 min.
test -x .augment/hooks/beads-prime.sh && test -x .augment/hooks/beads-sync.sh
for f in .augment/hooks/*.sh .augment/hooks/lib/*.sh \
  .augment/hooks/test/*.sh; do bash -n "$f"; done
python3 -m py_compile .augment/hooks/lib/command_scan.py
/bin/bash .augment/hooks/test/classification_test.sh
/bin/bash .augment/hooks/test/prime_test.sh
/bin/bash .augment/hooks/test/lock_test.sh
```
