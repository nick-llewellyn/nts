...
| `-t`, `--timeout <ms>`     | Per-request timeout, in milliseconds. Single global wall-clock budget shared across DNS, the NTS-KE handshake (TCP connect, TLS, record I/O), and the AEAD-NTPv4 UDP exchange as one shrinking deadline; the worst-case wall time tracks the value passed, not a multiple of it. | 5000    |
