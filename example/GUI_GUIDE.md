...
- **Get Time** — runs the high-level `getTime` flow: a warming handshake
  ...
  lowest-delay sample as a synchronized clock — the RFC 5905 peer
  delay when it falls inside the selection window
  `(0, roundTripMicros]`, which healthy samples normally do, else
