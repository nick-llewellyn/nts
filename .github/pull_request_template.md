...
- [ ] `(cd rust && cargo audit)` (any change to `rust/Cargo.toml` or `rust/Cargo.lock`;
      mirrors CI)
...
- [ ] `pubspec.yaml` `version:` left untouched. Versions are bumped
      only in a dedicated release PR, which also finalises the
      `CHANGELOG.md` header and carries no functional change
