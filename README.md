...
| `NtsError` | Sealed class: `invalidSpec`, `network`, `keProtocol`, `ntpProtocol`, `authentication`, `timeout(TimeoutPhase)`, `noCookies`, `trustBackendUnavailable`, `clockFault(stage: ClockFaultStage, fault: StrictClockError, generation)`, `internal`. `clockFault` means the clock the call was bound to could not vouch for the result; every entry point can raise it, with or without a `context`, because the native core runs its own clock reads strictly for every caller. |
