...
| `NtsBridge.state` | Which of `NtsBridgeState.uninitialized` / `.mock` / `.native` the bridge holds on this isolate. `.native` means calls dispatch across FFI into the Rust core; `.mock` means a caller-supplied API is installed. |
