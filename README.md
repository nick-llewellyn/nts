...
| `NtsProfile` | Tuning preset for `ntsGetTime` / `NtsClient.getTime`: `maxBurst`, `timeoutMs` (total budget for the whole call), `dnsConcurrencyCap`, `bridgeConcurrencyCap`. Presets: `mobile` (burst 4, 6 s, caps 4/4 — the default), `desktop` (burst 8, 7 s, caps 8/8), `embedded` (burst 2, 10 s, caps 2/2). |
