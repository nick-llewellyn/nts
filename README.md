...
2. **Symmetric-path delay compensation.** `utcUnixMicros` is the moment
   ...
   approximated as `utcUnixMicros` plus half the delay selected above.
   This is the standard NTP correction (RFC 5905 §8); it assumes the
   outbound and return paths are symmetric, which is why filtering on
   the lowest-delay sample matters — short paths are more likely to be
   symmetric.
...
| `ntsGetTimeStrict({required spec, required context, trustMode, customRoots, verificationTime})` | Strict counterpart of `ntsGetTime`: the same warm + burst and budget, with every clock decision made on the supplied `StrictClockContext` and no fallback at any of them — the budget is metered on it, each sample is attributed by its native receipt stamp rather than by plausibility, and the result is anchored on it. Returns `StrictSyncedTime`. A clock decision that cannot be made fails the call with `NtsError.clockFault`. Requires a `StrictClockProvenance.native` context in production. |
| `ntsQuery({required spec, timeout = kDefaultTimeout, dnsConcurrencyCap = kDefaultDnsConcurrencyCap, bridgeConcurrencyCap = kDefaultBridgeConcurrencyCap, verificationTime, context})` | Advanced primitive: one authenticated NTPv4 exchange. Returns `NtsTimeSample`. `verificationTime` (optional `DateTime`, interpreted as UTC, not before the epoch) pins TLS certificate validity-window checks to a fixed instant instead of the system clock — useful for cold-start clock-skew rescue. `context` (optional `StrictClockContext`) meters the bridge queue wait on that context instead of `MonotonicClock`, so a fault there fails the call as `NtsError.clockFault` rather than falling back. |
| `ntsWarmCookies({required spec, timeout = kDefaultTimeout, dnsConcurrencyCap = kDefaultDnsConcurrencyCap, bridgeConcurrencyCap = kDefaultBridgeConcurrencyCap, verificationTime, context})` | Advanced primitive: force a fresh NTS-KE handshake. Returns `NtsWarmCookiesOutcome`. `verificationTime` and `context` carry the same semantics as on `ntsQuery`. |
...
| `NtsClient({trustMode = TrustMode.platformWithFallback, customRoots})` | Owned client with its own per-host session table — cookies, AEAD keys, and KE sessions are isolated from the default singleton and from other clients. Methods: `query` / `warmCookies` / `getTime` / `getTimeStrict` (per-client equivalents of the top-level functions, same parameters including `verificationTime` and `context`), `invalidate(spec)` (drop one cached session) / `clear()` (drop all), `dispose()` (release the native handle and the whole session table eagerly instead of waiting for the GC finalizer — optional, idempotent, and distinct from `clear()` in that it ends the client rather than emptying it), and the `trustMode` getter. `customRoots` is required (and only valid) when `trustMode` is `TrustMode.custom`. |
...
| `StrictSyncedTime` | Synchronized clock returned by `getTimeStrict`, bound to the `StrictClockContext` the call ran on. Same sample statistics as `NtsSyncedTime` (`utcUnixMicros`, `roundTripMicros`, `samplesUsed`, `trustBackend`, `offsetMicros`, `jitterMicros`, `errorBoundMicros`) plus the anchor's coordinate: `descriptor`, `generation`, `anchorMicros`. `utcNow()` / `elapsedSinceSync()` re-read the bound context and throw the `StrictClockError` that read raises — `StrictClockInvalidated` once the context has been retired — instead of falling back. `isValid` reports only invalidations already observed, as on `StrictClockContext`. |
...
| `StrictClockContext` | **Preferred strict sleep-aware clock.** Where `MonotonicClock` is best-effort and carries no provenance, `StrictClockContext.resolve()` binds one explicit `ClockSourceDescriptor` and one live `generation` on the native bridge, with `provenance` `StrictClockProvenance.native`. `now()` returns a `StrictReading` on that coordinate; `elapsedSince(reading)` returns a `Duration` from any reading taken through the same bridge incarnation on the same coordinate and generation, rejecting a foreign one (`StrictClockSourceIncompatible` / `StrictClockDescriptorIncompatible` / `StrictClockGenerationIncompatible`) without reading the clock. A source fault or a regression throws a `StrictClockError` on that call and leaves the context permanently invalid; a lifecycle event (bridge reset, native generation advance, explicit `invalidate()`) makes the next `now()` / `elapsedSince()` throw `StrictClockInvalidated` — there is no `Stopwatch` fallback and no repair short of resolving a new context. `isValid` and `invalidationReason` report only invalidations the context has already observed — an explicit `invalidate()`, or a call that threw — not liveness: a bridge reset or generation advance not yet seen by a read leaves them unchanged until the next call fails. `resolve()` fails distinctly on an uninitialized bridge (`StrictClockUninitialized`), a hand-written mock (`StrictClockMockOnly`; tests use `resolveForTesting()`, whose contexts carry `testInjected` provenance) and an unsupported platform (`StrictClockUnsupported`). `exportReference(time, provider:)` / `bindReference(reference, provider:)` are the same-boot transfer surface: they carry a `StrictSyncedTime`'s reference reading — the winning sample's wire receipt, on an instance from `getTimeStrict` — between engines or processes of one boot as a `SameBootReference`. `exportReference` checks that the instance is bound to the exporting context; the public `StrictSyncedTime` constructor means wire-receipt provenance is the claim of whoever built the instance, so what is exported is its reference reading, which is always an instant that context attributed and that orders at or before a read taken during the export. `bindReference` first refuses a reference whose coordinate is not exactly compatible with `StrictClockDescriptorIncompatible` — an inexact mapping is never converted — and an unprovable boot scope is then refused with `StrictClockBootScopeUnavailable`; both leave the context valid, while a lifecycle or read failure invalidates it as any failed read does. Both are **explicitly unavailable as shipped** — `kApprovedBootScopeProviders` is empty, so a native context refuses with `BootScopeUnavailableReason.providerNotApproved` every call that reaches the provider step; the checks that precede it still fire first, so a `time` bound to another context is an `ArgumentError` and an incompatible reference a `StrictClockDescriptorIncompatible`. Local strict reads are unaffected. |
| `StrictClockError` | Sealed hierarchy every strict clock failure belongs to, for exhaustive `switch`ing. Thrown by `StrictClockContext.resolve()`, `now()`, `elapsedSince()`, `exportReference()` and `bindReference()` and by the `StrictSyncedTime` projections (`resolveForTesting()` throws the same errors once past its mock-only precondition; violating that precondition is a test-setup `StateError` outside the hierarchy): `StrictClockUninitialized`, `StrictClockMockOnly`, `StrictClockUnsupported`, `StrictClockSourceFault(kind: SourceFaultKind, detail: String, errno?)` (`errno` present exactly for `SourceFaultKind.syscallFailed`; the constructor throws `ArgumentError` on any other pairing, at runtime in every build mode), `StrictClockRegression(previous, observed)`, `StrictClockInvalidated(generation, reason: StrictClockInvalidationReason)`, `StrictClockUnknownSource`, `StrictClockSourceIncompatible`, `StrictClockDescriptorIncompatible`, `StrictClockGenerationIncompatible`, `StrictClockBootScopeUnavailable(reason: BootScopeUnavailableReason, providerId)` (raised only by `exportReference` / `bindReference`; it reports that same-boot scope could not be proven, not that the clock failed, so — like the three `Incompatible` subtypes before it, which are likewise raised without reading the clock — it leaves the context valid). Three further subtypes arise only while a call acquires a sample and reach the caller as the `fault` of an `NtsError.clockFault`, never from a context method: `StrictClockSuspendedInFlight` (the native core's per-sample verdict at `ClockFaultStage.receipt` that the device slept between send and receive — checked for every caller, so `ntsQuery` surfaces it with or without a `context`), and `StrictClockMissingReceipt` / `StrictClockForeignReceipt` (a sample with no native receipt stamp, or one from another generation or backend, at `ClockFaultStage.attribution` — raised only by the strict `getTimeStrict` surfaces, which are the only ones that attribute). A failure is only ever thrown, never returned in place of a reading. Two failures on the transfer surface sit outside the hierarchy, so an exhaustive `StrictClockError` catch does not cover them: the `ArgumentError` `exportReference` throws for a `StrictSyncedTime` another context acquired, and whatever `BootScopeProvider.current()` throws, which propagates unchanged — the transfer is abandoned and nothing is exported, adopted or invalidated. |
| `ClockSourceDescriptor` | The coordinate a `StrictClockContext` is bound to: `backend` (`ClockBackend.linuxBoottime` / `appleContinuous` / `windowsInterruptTime`), `semanticsVersion`, `conversionVersion`. Two descriptors are compatible when all three match, independent of package version. |
| `NtsTimeSample` | `utcUnixMicros`, `roundTripMicros`, `serverStratum`, `aeadId`, `freshCookies`, `phaseTimings`, `trustBackend`, `recvBoottimeMicros` (sleep-aware wire-level receipt stamp, taken as a strict read immediately after the UDP recv in the native worker; per-boot epoch, never persist), `recvClockGeneration` and `recvClockBackend` (the strict-clock generation and backend that read took — a stamp is attributed by comparing them with a `StrictClockContext`'s own, not by its numeric plausibility and not by assuming it shares `MonotonicClock`'s coordinate, since the best-effort clock can latch its `Instant` fallback while strict receipts stay on the native clock; `0` / `null` mark a fixture with no strict receipt), plus the RFC 5905 statistics `offsetMicros` (θ), `peerDelayMicros` (δ), `rootDelayMicros`, `rootDispersionMicros`, `serverPrecision`, plus `keWarnings` (non-fatal NTS-KE warning codes from the establishing handshake, RFC 8915 §4.1.4 record type 3, as raw `int` values — empty for every server observed in practice, since the IANA registry has no assignments as of RFC 8915). `roundTripMicros` is the UDP-phase wall-clock cost; the four pre-NTP phases live on `phaseTimings`; `trustBackend` records which trust-anchor backend the post-handshake TLS verification chose. `keWarnings` describes the handshake rather than the call, so like `trustBackend` — and unlike `phaseTimings` — it follows the session across cached-session queries instead of resetting to empty. |
...
| `NtsError` | Sealed class: `invalidSpec`, `network`, `keProtocol`, `ntpProtocol`, `authentication`, `timeout(TimeoutPhase)`, `noCookies`, `trustBackendUnavailable`, `internal`, `abiMismatch`, `clockFault(stage: ClockFaultStage, fault: StrictClockError, generation, trustBackend?)`. `clockFault` means the clock the call was bound to could not vouch for the result; every entry point can raise it, with or without a `context`, because the native core runs its own clock reads strictly for every caller. |
| `ClockFaultStage` | `admission`, `handshake`, `session`, `udp`, `receipt`, `awaitResult`, `attribution`, `projection`. Carried on `NtsError.clockFault` to name where in the call the read that faulted served. The first five are native stages any call can report; the last three are Dart-authored and fire only on the strict surfaces (`ntsGetTimeStrict`, `NtsClient.getTimeStrict`, or `ntsQuery` / `ntsWarmCookies` given `context`). |
