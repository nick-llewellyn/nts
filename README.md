...
| `NtsSyncedTime` | Synchronized clock returned by `getTime`: `utcUnixMicros` (compensated best sample at the anchor), `offsetMicros` (synced minus system clock at the anchor), `roundTripMicros` (winning sample), `samplesUsed`, `trustBackend`, `utcNow` (monotonic projection immune to system clock changes), `elapsedSinceSync`. Identity semantics — a live clock, not a value-type DTO. |
